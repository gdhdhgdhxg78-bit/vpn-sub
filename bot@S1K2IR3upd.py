...
TEXT_NAMES = {
    ...
    "banned_user": "متن کاربر مسدود شده",
...
def delete_all_configs() -> int:
    """حذف کل کانفیگ‌های ذخیره‌شده در دیتابیس و برگرداندن تعداد حذف‌شده‌ها."""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def get_all_missions(active_only=False):
    ...
    c = conn.cursor()
    if active_only:
        c.execute("SELECT id, title, description, reward_coins, mission_type, target_count, is_active FROM missions WHERE is_active=1 ORDER BY id ASC")
    else:
        c.execute("SELECT id, title, description, reward_coins, mission_type, target_count, is_active FROM missions ORDER BY id ASC")
    rows = c.fetchall(); conn.close(); return rows

def get_mission(mission_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, title, description, reward_coins, mission_type, target_count, is_active FROM missions WHERE id=?", (mission_id,))
    row = c.fetchone(); conn.close(); return row

def add_mission(title, description, reward_coins, mission_type, target_count):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO missions (title, description, reward_coins, mission_type, target_count, is_active, created_at) VALUES (?,?,?,?,?,1,?)",
              (title, description, reward_coins, mission_type, target_count, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit(); conn.close()
...
def delete_mission(mission_id):
    ...
    c.execute("DELETE FROM user_missions WHERE mission_id=?", (mission_id,))
    conn.commit(); conn.close()

def toggle_mission_active(mission_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE missions SET is_active = CASE WHEN is_active=1 THEN 0 ELSE 1 END WHERE id=?", (mission_id,))
    conn.commit(); conn.close()

def get_mission_progress(user_id, mission):
    mid, title, desc, reward, mtype, target, is_active = mission
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    if mtype == "invite":
        c.execute("SELECT COUNT(*) FROM users WHERE referred_by=?", (user_id,))
        ...
        c.execute("SELECT configs_received FROM users WHERE user_id=?", (user_id,))
        row = c.fetchone(); progress = row[0] if row else 0
    elif mtype == "coins":
        c.execute("SELECT coins FROM users WHERE user_id=?", (user_id,))
        row = c.fetchone(); progress = row[0] if row else 0
    else:
        progress = 0
    c.execute("SELECT claimed FROM user_missions WHERE user_id=? AND mission_id=?", (user_id, mid))
    row = c.fetchone()
    claimed = row[0] if row else 0
    conn.close()
    return progress, claimed

def claim_mission_reward(user_id, mission_id):
    mission = get_mission(mission_id)
    if not mission: return False, "ماموریت پیدا نشد"
    mid, title, desc, reward, mtype, target, is_active = mission
    progress, claimed = get_mission_progress(user_id, mission)
    if claimed: return False, "claimed"
    if progress < target: return False, "not_done"
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO user_missions (user_id, mission_id, claimed, claimed_at) VALUES (?,?,1,?)",
              (user_id, mission_id, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    c.execute("UPDATE users SET coins = coins + ? WHERE user_id=?", (reward, user_id))
    conn.commit(); conn.close()
    return True, reward

# ==================== توابع نفرات برتر ====================
def get_leaderboard(metric="refs", limit=10):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    if metric == "refs":
        c.execute("""SELECT u.user_id, u.full_name,
                     (SELECT COUNT(*) FROM users WHERE referred_by=u.user_id) AS val
                     FROM users u WHERE u.is_banned=0
                     ORDER BY val DESC LIMIT ?""", (limit,))
    elif metric == "coins":
        c.execute("SELECT user_id, full_name, coins FROM users WHERE is_banned=0 ORDER BY coins DESC LIMIT ?", (limit,))
    elif metric == "configs":
        c.execute("SELECT user_id, full_name, configs_received FROM users WHERE is_banned=0 ORDER BY configs_received DESC LIMIT ?", (limit,))
    rows = c.fetchall(); conn.close(); return rows

def get_user_rank(user_id, metric="refs"):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    if metric == "refs":
        c.execute("""SELECT COUNT(*)+1 FROM users u WHERE is_banned=0 AND
                     (SELECT COUNT(*) FROM users WHERE referred_by=u.user_id) >
                     (SELECT COUNT(*) FROM users WHERE referred_by=?)""", (user_id,))
    elif metric == "coins":
        c.execute("SELECT COUNT(*)+1 FROM users WHERE is_banned=0 AND coins > (SELECT coins FROM users WHERE user_id=?)", (user_id,))
    elif metric == "configs":
        c.execute("SELECT COUNT(*)+1 FROM users WHERE is_banned=0 AND configs_received > (SELECT configs_received FROM users WHERE user_id=?)", (user_id,))
...
def main_keyboard(show_admin_btn: bool = False) -> InlineKeyboardMarkup:
    rows = [
        ...
        [
            InlineKeyboardButton(text=get_button_text("main_missions"), callback_data="missions", style=style_of("main_missions")),
            InlineKeyboardButton(text=get_button_text("main_leaderboard"), callback_data="leaderboard", style=style_of("main_leaderboard")),
        ],
        [
...
def admin_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if has_perm(user_id, "leave_alert"):
        rows.append([InlineKeyboardButton(text=get_button_text("admin_leave_alert"), callback_data="admin_leave_alert")])
    ...
    if has_perm(user_id, "missions"):
        rows.append([InlineKeyboardButton(text=get_button_text("admin_missions"), callback_data="admin_missions_0")])
    ...
    if has_perm(user_id, "premium_emojis"):
        rows.append([InlineKeyboardButton(text=get_button_text("admin_premium_emojis"), callback_data="admin_premium_emojis")])
...
def join_keyboard() -> InlineKeyboardMarkup:
    ...
    for ch in CHANNEL_IDS:
        label = f"{prefix} {ch}".strip() if prefix else ch
        buttons.append([InlineKeyboardButton(text=label, url=f"https://t.me/{ch.lstrip('@')}", style=style_of("join_channel"))])
...
async def on_chat_member_update(event):
    ...
    try:
        ...
        if chat_username not in CHANNEL_IDS and str(chat.id) not in CHANNEL_IDS:
            return
...
async def cb_admin_delete_all_configs(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        f"⚠️ هشدار: با تایید، تمام کانفیگ‌های موجود (مصرف‌شده و آماده) از دیتابیس پاک می‌شن. این عملیات بازگشت‌پذیر نیست.\n\nمطمئنی؟",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
...
# ==================== ماموریت‌ها (کاربر) ====================
def _missions_keyboard_user(user_id: int) -> InlineKeyboardMarkup:
    missions = get_all_missions(active_only=True)
    rows = []
    for m in missions:
        mid, title, desc, reward, mtype, target, is_active = m
        progress, claimed = get_mission_progress(user_id, m)
        done = progress >= target
        if claimed:
            label = f"✅ {title}"
        elif done:
            label = f"🎁 {title} — آماده دریافت!"
        else:
            label = f"{'▓' * int(10 * min(progress, target) / target)}{'░' * (10 - int(10 * min(progress, target) / target))} {progress}/{target}  |  {title}"
        rows.append([InlineKeyboardButton(text=label, callback_data=f"mission_view_{mid}")])
    rows.append([InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def cb_missions(call: CallbackQuery):
    ...
    missions = get_all_missions(active_only=True)
    if not missions:
        await call.message.edit_text(get_bot_text("missions_empty"),
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")]]))
        return
    await call.message.edit_text(get_bot_text("missions_header"),
        reply_markup=_missions_keyboard_user(call.from_user.id))

@dp.callback_query(F.data.startswith("mission_view_"))
async def cb_mission_view(call: CallbackQuery):
    await call.answer()
    try:
    ...
    if not mission or not mission[6]:
        await call.answer("این ماموریت دیگه وجود نداره.", show_alert=True); return
    mid, title, desc, reward, mtype, target, is_active = mission
    progress, claimed = get_mission_progress(call.from_user.id, mission)
    done = progress >= target
    type_labels = {"invite": "👥 دعوت دوستان", "configs": "📦 دریافت اتصال", "coins": "🪙 جمع‌آوری سکه"}
    bar_filled = int(10 * min(progress, target) / target) if target > 0 else 0
    bar = "▓" * bar_filled + "░" * (10 - bar_filled)
    status = "✅ دریافت شد" if claimed else ("🎁 آماده دریافت!" if done else "⏳ در حال انجام")
    text = (f"🎯 {title}\n━━━━━━━━━━━━━━━\n"
            f"📝 {desc}\n\n"
            f"🏷 نوع: {type_labels.get(mtype, mtype)}\n"
            f"🎁 جایزه: {reward} سکه 🪙\n"
            f"🎯 هدف: {target}\n\n"
    ...
    if done and not claimed:
        kb_rows.append([InlineKeyboardButton(text=get_button_text("mission_claim"), callback_data=f"mission_claim_{mid}", style=style_of("mission_claim"))])
    kb_rows.append([InlineKeyboardButton(text="🔙 بازگشت به ماموریت‌ها", callback_data="missions")])
...
async def cb_mission_claim(call: CallbackQuery):
    try:
        mid = int(call.data.split("_")[-1])
    except:
        await call.answer(); return
    success, result = claim_mission_reward(call.from_user.id, mid)
    if not success:
        if result == "claimed":
            await call.answer(get_bot_text("mission_already_claimed"), show_alert=True)
        elif result == "not_done":
            mission = get_mission(mid)
            if mission:
                progress, _ = get_mission_progress(call.from_user.id, mission)
                tmpl = get_bot_text("mission_not_done")
                try:
                    msg = tmpl.format(progress=progress, target=mission[5])
                except:
                    msg = tmpl
                await call.answer(msg, show_alert=True)
            else:
                await call.answer("❌ خطا در دریافت جایزه.", show_alert=True)
        else:
            await call.answer(f"❌ {result}", show_alert=True)
        return
    mission = get_mission(mid)
    title = mission[1] if mission else "ماموریت"
    reward = result
    await call.answer(f"🎉 +{reward} سکه دریافت شد!", show_alert=True)
    tmpl = get_bot_text("mission_completed_notif")
    try:
        notif = tmpl.format(title=title, reward=reward)
    except:
        notif = tmpl
    await call.message.edit_text(notif,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎯 بازگشت به ماموریت‌ها", callback_data="missions")],
            [InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")]]))

# ==================== نفرات برتر (کاربر) ====================
def _leaderboard_medals(rank: int) -> str:
    return {1: "🥇", 2: "🥈", 3: "🥉"}.get(rank, f"#{rank}")

def _build_leaderboard_text(metric: str, user_id: int) -> str:
    header_key = {"refs": "leaderboard_header_refs", "coins": "leaderboard_header_coins", "configs": "leaderboard_header_configs"}.get(metric, "leaderboard_header_refs")
    rows = get_leaderboard(metric, 10)
    value_suffix = {"refs": " دعوت", "coins": " سکه", "configs": " اتصال"}.get(metric, "")
    text = get_bot_text(header_key)
    for i, row in enumerate(rows, 1):
        uid, name, val = row
        medal = _leaderboard_medals(i)
        safe_name = (name or "کاربر")[:20]
        text += f"{medal} {safe_name} — {val}{value_suffix}\n"
    if not rows:
        text += "هنوز کسی ثبت نشده!\n"
    user_rank = get_user_rank(user_id, metric)
    if user_rank and user_rank <= 10:
        rank_tmpl = get_bot_text("leaderboard_your_rank")
        try:
            text += rank_tmpl.format(rank=user_rank)
        except:
            text += rank_tmpl
    else:
        text += get_bot_text("leaderboard_not_ranked")
    return text

@dp.callback_query(F.data == "leaderboard")
async def cb_leaderboard(call: CallbackQuery):
    await call.answer()
    text = _build_leaderboard_text("refs", call.from_user.id)
    await call.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text=get_button_text("leaderboard_by_refs"), callback_data="leaderboard_refs"),
            InlineKeyboardButton(text=get_button_text("leaderboard_by_coins"), callback_data="leaderboard_coins"),
        ],
        [InlineKeyboardButton(text=get_button_text("leaderboard_by_configs"), callback_data="leaderboard_configs")],
        [InlineKeyboardButton(text=get_button_text("back_main"), callback_data="back_main")],
    ]))

@dp.callback_query(F.data.startswith("leaderboard_") & ~F.data.startswith("leaderboard_by"))
async def cb_leaderboard_metric(call: CallbackQuery):
    await call.answer()
    metric = call.data.replace("leaderboard_", "")
    if metric not in ("refs", "coins", "configs"):
        return
    text = _build_leaderboard_text(metric, call.from_user.id)
    await call.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text=get_button_text("leaderboard_by_refs"), callback_data="leaderboard_refs"),
            InlineKeyboardButton(text=get_button_text("leaderboard_by_coins"), callback_data="leaderboard_coins"),
...
# ==================== مدیریت ماموریت‌ها (ادمین) ====================
def _admin_missions_keyboard(page=0) -> InlineKeyboardMarkup:
    missions = get_all_missions()
    per_page = 5
    start = page * per_page
    chunk = missions[start:start + per_page]
    rows = []
    for m in chunk:
        mid, title, desc, reward, mtype, target, is_active = m
        status_icon = "✅" if is_active else "🔴"
        rows.append([InlineKeyboardButton(text=f"{status_icon} {title} | {reward}🪙", callback_data=f"admin_mission_view_{mid}")])
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text=get_button_text("prev_page"), callback_data=f"admin_missions_{page-1}"))
    if start + per_page < len(missions):
        nav.append(InlineKeyboardButton(text=get_button_text("next_page"), callback_data=f"admin_missions_{page+1}"))
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton(text=get_button_text("mission_add"), callback_data="admin_mission_add")])
    rows.append([InlineKeyboardButton(text=get_button_text("back_panel"), callback_data="open_admin_panel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)

@dp.callback_query(F.data.startswith("admin_missions_"))
async def cb_admin_missions(call: CallbackQuery, state: FSMContext):
    await state.clear()
    if not has_perm(call.from_user.id, "missions"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    try:
        page = int(call.data.split("_")[-1])
    ...
    missions = get_all_missions()
    await call.message.edit_text(
        f"🎯 مدیریت ماموریت‌ها\n━━━━━━━━━━━━━━━\n\nتعداد ماموریت‌ها: {len(missions)}\n\nیک ماموریت رو انتخاب کن یا ماموریت جدید اضافه کن:",
        reply_markup=_admin_missions_keyboard(page))

@dp.callback_query(F.data.startswith("admin_mission_view_"))
async def cb_admin_mission_view(call: CallbackQuery):
    if not has_perm(call.from_user.id, "missions"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    try:
    ...
    m = get_mission(mid)
    if not m:
        await call.answer("ماموریت پیدا نشد.", show_alert=True); return
    mid, title, desc, reward, mtype, target, is_active = m
    type_labels = {"invite": "👥 دعوت دوستان", "configs": "📦 دریافت اتصال", "coins": "🪙 جمع‌آوری سکه"}
    ...
    text = (f"🎯 {title}\n━━━━━━━━━━━━━━━\n"
            ...
            f"🎁 جایزه: {reward} سکه\n"
            f"📌 وضعیت: {status}")
    toggle_text = get_button_text("mission_toggle_inactive") if is_active else get_button_text("mission_toggle_active")
    await call.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=toggle_text, callback_data=f"admin_mission_toggle_{mid}")],
        [InlineKeyboardButton(text=get_button_text("mission_delete"), callback_data=f"admin_mission_delete_{mid}")],
        [InlineKeyboardButton(text="🔙 بازگشت به ماموریت‌ها", callback_data="admin_missions_0")],
    ]))

@dp.callback_query(F.data.startswith("admin_mission_toggle_"))
async def cb_admin_mission_toggle(call: CallbackQuery):
    if not has_perm(call.from_user.id, "missions"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    try:
        mid = int(call.data.split("_")[-1])
        ...
        await call.answer(); return
    toggle_mission_active(mid)
    await call.answer("✅ وضعیت ماموریت تغییر کرد.")
    m = get_mission(mid)
    if not m:
        await call.message.edit_text("ماموریت پیدا نشد.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin_missions_0")]])); return
    mid, title, desc, reward, mtype, target, is_active = m
    type_labels = {"invite": "👥 دعوت دوستان", "configs": "📦 دریافت اتصال", "coins": "🪙 جمع‌آوری سکه"}
    status = "✅ فعال" if is_active else "🔴 غیرفعال"
    text = (f"🎯 {title}\n━━━━━━━━━━━━━━━\n"
...
@dp.callback_query(F.data.startswith("admin_mission_delete_"))
async def cb_admin_mission_delete(call: CallbackQuery):
    if not has_perm(call.from_user.id, "missions"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    try:
        mid = int(call.data.split("_")[-1])
    ...
    if not m:
        await call.answer("ماموریت پیدا نشد.", show_alert=True); return
    delete_mission(mid)
    await call.answer("🗑 ماموریت حذف شد.")
    await call.message.edit_text("🗑 ماموریت حذف شد.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 بازگشت به ماموریت‌ها", callback_data="admin_missions_0")]]))

@dp.callback_query(F.data == "admin_mission_add")
async def cb_admin_mission_add(call: CallbackQuery, state: FSMContext):
    if not has_perm(call.from_user.id, "missions"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    await call.answer()
    await state.set_state(AdminStates.mission_add_title)
    await call.message.edit_text(
        "🎯 ماموریت جدید\n━━━━━━━━━━━━━━━\n\nمرحله ۱/۵ — اسم ماموریت رو بفرست:\n\nمثال: دعوت ۵ دوست",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("cancel_action"), callback_data="admin_missions_0")]]))

@dp.message(AdminStates.mission_add_title)
async def hdl_mission_add_title(message: Message, state: FSMContext):
    if not has_perm(message.from_user.id, "missions"):
        await state.clear(); return
    title = message.text.strip() if message.text else ""
    if not title:
        await message.answer("❌ اسم نمی‌تونه خالی باشه. دوباره بفرست:"); return
    await state.update_data(mission_title=title)
    await state.set_state(AdminStates.mission_add_desc)
    await message.answer(
        "مرحله ۲/۵ — توضیح کوتاه ماموریت رو بفرست:\n\nمثال: ۵ نفر از دوستاتو دعوت کن و سکه بگیر!\n(اگه نمی‌خوای بزن - )",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("cancel_action"), callback_data="admin_missions_0")]]))

@dp.message(AdminStates.mission_add_desc)
async def hdl_mission_add_desc(message: Message, state: FSMContext):
    if not has_perm(message.from_user.id, "missions"):
        await state.clear(); return
    desc = message.text.strip() if message.text else ""
    if desc == "-": desc = ""
    await state.update_data(mission_desc=desc)
    await state.set_state(AdminStates.mission_add_type)
    await message.answer(
        "مرحله ۳/۵ — نوع ماموریت رو انتخاب کن:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="👥 دعوت دوستان (invite)", callback_data="mtype_invite")],
            [InlineKeyboardButton(text="📦 دریافت اتصال (configs)", callback_data="mtype_configs")],
            [InlineKeyboardButton(text="🪙 جمع‌آوری سکه (coins)", callback_data="mtype_coins")],
            [InlineKeyboardButton(text=get_button_text("cancel_action"), callback_data="admin_missions_0")],
        ]))

@dp.callback_query(F.data.startswith("mtype_"))
async def cb_mission_type_select(call: CallbackQuery, state: FSMContext):
    if not has_perm(call.from_user.id, "missions"):
        await call.answer("❌ دسترسی ندارید.", show_alert=True); return
    mtype = call.data.replace("mtype_", "")
    if mtype not in ("invite", "configs", "coins"):
        await call.answer(); return
    await call.answer()
    await state.update_data(mission_type=mtype)
    await state.set_state(AdminStates.mission_add_target)
    type_labels = {"invite": "تعداد دعوت", "configs": "تعداد اتصال", "coins": "تعداد سکه"}
    await call.message.edit_text(
        f"مرحله ۴/۵ — هدف ({type_labels.get(mtype, 'تعداد')}) رو بفرست:\n\nمثال: 5",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("cancel_action"), callback_data="admin_missions_0")]]))

@dp.message(AdminStates.mission_add_target)
async def hdl_mission_add_target(message: Message, state: FSMContext):
    if not has_perm(message.from_user.id, "missions"):
        await state.clear(); return
    try:
        ...
        await message.answer("❌ عدد معتبر بفرست (مثلاً 5):"); return
    await state.update_data(mission_target=target)
    await state.set_state(AdminStates.mission_add_reward)
    await message.answer(
        "مرحله ۵/۵ — جایزه (تعداد سکه) رو بفرست:\n\nمثال: 20",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=get_button_text("cancel_action"), callback_data="admin_missions_0")]]))

@dp.message(AdminStates.mission_add_reward)
async def hdl_mission_add_reward(message: Message, state: FSMContext):
    if not has_perm(message.from_user.id, "missions"):
        await state.clear(); return
    try:
        ...
        await message.answer("❌ عدد معتبر بفرست (مثلاً 20):"); return
    data = await state.get_data()
    title = data.get("mission_title", "")
    desc = data.get("mission_desc", "")
    mtype = data.get("mission_type", "invite")
    target = data.get("mission_target", 1)
    add_mission(title, desc, reward, mtype, target)
    await state.clear()
    type_labels = {"invite": "👥 دعوت دوستان", "configs": "📦 دریافت اتصال", "coins": "🪙 جمع‌آوری سکه"}
    await message.answer(
        f"✅ ماموریت جدید ذخیره شد!\n\n"
        f"🎯 اسم: {title}\n"
        f"📝 توضیح: {desc or '—'}\n"
        f"🏷 نوع: {type_labels.get(mtype, mtype)}\n"
        f"🎯 هدف: {target}\n"
